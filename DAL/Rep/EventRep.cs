    ...
    public class EventRep
    {
            ...
            using (CDBContext db = new CDBContext())
            {
                ...
                if (Elist != null)
                {
                    foreach (var item in Elist)
                    {
                        ...
                        if (userId.HasValue)
                        {
                            var subEvent = db.Subscribes.Where(x => x.UserId == userId && x.EventId == item.EventId).FirstOrDefault();
                            if (subEvent != null)
                            {
                                data.IsLike = subEvent.Is_Like;
                            } 
                        }
        ...
        public EventDto GetEventById(int id)
        {
...

            {
                ...
                if (PDto.EventName == null)
                {
                    PDto.ProviderName = db.Providers.Where(x => x.ProviderId == P.ProviderId).Select(x => x.Name).FirstOrDefault();
                }
